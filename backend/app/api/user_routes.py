...
from app.models.schemas import ApplicationSubmitRequest, UserCredentials
...
async def submit_application(payload: ApplicationSubmitRequest, current=Depends(get_current_user)):
    try:
        ...
        departments_needed = ["Municipality"]
        if payload.crowd_size > 200 or payload.is_moving_procession:
            departments_needed.append("Police")
        if payload.has_fireworks:
            departments_needed.append("Fire")

        routing_inserts = [
            {"app_id": custom_app_id, "department": d, "status": "Pending"}
